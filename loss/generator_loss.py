...
def compute_generator_losses(
    ...
    for i in range(len(Xt_attr)):
        L_attr += torch.mean(
            torch.pow(Xt_attr[i] - Y_attr[i], 2).reshape(args.batch_size, -1), dim=1
        ).mean()
    L_attr /= 2.0
    ...
    L_rec = torch.sum(
        0.5
        * torch.mean(torch.pow(Y - Xt, 2).reshape(args.batch_size, -1), dim=1)
        * same_person
    ) / (same_person.sum() + 1e-6)

    # l2 eyes loss
    if args.eye_detector_loss:
        Xt_heatmap_left, Xt_heatmap_right, Y_heatmap_left, Y_heatmap_right = (
            eye_heatmaps
        )
        L_l2_eyes = F.mse_loss(Xt_heatmap_left, Y_heatmap_left) + F.mse_loss(
            Xt_heatmap_right, Y_heatmap_right
        )
    else:
        L_l2_eyes = 0
    ...
    lossG = (
        args.weight_adv * L_adv
        + args.weight_attr * L_attr
        + args.weight_id * L_id
        + args.weight_rec * L_rec
        + args.weight_eyes * L_l2_eyes
    )
